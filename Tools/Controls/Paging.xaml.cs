...
namespace WpfApp.Tools.Controls
{
    /// <summary>
    /// Paging.xaml 的交互逻辑
    /// </summary>
    public partial class Paging : UserControl
    {
        ...
        private void setPageSizeBtn_Click(object sender, RoutedEventArgs e)
        {
            try
            {
                ...
                //为空：返回
                if (pageSizeTb.Text.Trim() == "")
                {
                    MessageBox.Show("请输入要显示的条数", "系统提示！", MessageBoxButton.OK, MessageBoxImage.Stop);
                    return;
                }               
                if (System.Convert.ToInt32(pageSizeTb.Text) <= 0)
                {
                    MessageBox.Show("条数不能为负数", "系统提示！", MessageBoxButton.OK, MessageBoxImage.Stop);
                    return;
                }
